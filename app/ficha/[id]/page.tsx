...
export default async function FichaSIGA({ params }: { params: Promise<{ id: string }> }) {
  ...
  const estiloBox: React.CSSProperties = {
    border: '1px solid #000', minHeight: 28, padding: '4px 8px',
    fontSize: 13, marginBottom: 12, color: '#000', background: '#fff',
  }
  const estiloGrid = (cols: string): React.CSSProperties => ({
    display: 'grid', gridTemplateColumns: cols, gap: 12, marginBottom: 4,
  })
    ...
    <div style={{ background: '#fff', minHeight: '100vh', color: '#000' }}>
        ...
        * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
        @media print {
          ...
          * { color: #000 !important; background: #fff !important; border-color: #000 !important; }
          .ficha-box { border: 1px solid #000 !important; background: #fff !important; }
          .checkbox-marcado { background: #000 !important; }
        }
      ...
      <div style={{ maxWidth: 800, margin: '32px auto', padding: '0 32px 40px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24, borderBottom: '2px solid #000', paddingBottom: 12 }}>
          <p style={{ fontWeight: 700, fontSize: 14, margin: 0, letterSpacing: 1, color: '#000' }}>CONGREGAÇÃO CRISTÃ NO BRASIL</p>
          <p style={{ fontSize: 12, margin: '4px 0 0', color: '#000' }}>Departamento de Assistência Religiosa para Evangelização - DARPE</p>
          <p style={{ fontSize: 12, margin: '8px 0 0', fontWeight: 600, color: '#000' }}>04-Setor Hospital</p>
        </div>

        <h2 style={{ textAlign: 'center', fontSize: 20, fontWeight: 700, margin: '0 0 24px', letterSpacing: 1, color: '#000' }}>Ficha Cadastral</h2>
        ...
        <div style={{ marginBottom: 20, marginTop: 8 }}>
          <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', marginBottom: 10, color: '#000' }}>
            Função dos Atendimentos do DARPE
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {['Ministério', 'Atendente', 'Colaborador', 'Encarregado', 'Músico/Organista', 'Grupo de Canto'].map(f => (
              <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div className={m.funcao_darpe === f ? 'checkbox-marcado' : ''} style={{
                  width: 14, height: 14,
                  border: '1px solid #000',
                  background: m.funcao_darpe === f ? '#000' : '#fff',
                  flexShrink: 0,
                }} />
                <span style={{ fontSize: 12, color: '#000' }}>{f}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Declaração */}
        <div className="ficha-box" style={{
          border: '1px solid #000', padding: '10px 12px',
          marginBottom: 24, fontSize: 11, lineHeight: 1.5,
        }}>
          <p style={{ fontWeight: 700, marginBottom: 6, fontSize: 11, color: '#000' }}>DECLARAÇÃO DE VOLUNTARIADO</p>
        ...
        {/* Data */}
        <p style={{ textAlign: 'center', fontSize: 13, marginBottom: 40, color: '#000' }}>
          ...
          {['Ministério', 'Encarregado de Orquestra', 'Voluntário'].map(cargo => (
            <div key={cargo} style={{ textAlign: 'center' }}>
              <div style={{ borderTop: '1px solid #000', paddingTop: 6 }}>
                <p style={{ fontSize: 11, margin: 0, fontWeight: 600, color: '#000' }}>{cargo}</p>
                <p style={{ fontSize: 11, margin: '4px 0 0', color: '#000' }}>Nome: _______________________</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Rodapé */}
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 40, borderTop: '1px solid #000', paddingTop: 8 }}>
          <span style={{ fontSize: 10, color: '#000' }}>SIGA - DARPE</span>
          <span style={{ fontSize: 10, color: '#000' }}>Folha 1 / 1</span>
        </div>
