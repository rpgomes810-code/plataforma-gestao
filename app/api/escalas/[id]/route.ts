...
async function notificarAtendente(nomeAtendente: string, grupo: string, data: string, hospital: string) {
  try {
    const { data: membro } = await supabaseAdmin.from('membros').select('id').eq('nome', nomeAtendente).single()
    if (!membro) return
    const { data: assinatura } = await supabaseAdmin.from('push_subscriptions').select('subscription').eq('membro_id', membro.id).single()
    if (!assinatura) return
    webpush.setVapidDetails(process.env.VAPID_SUBJECT!, process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY!, process.env.VAPID_PRIVATE_KEY!)
    await webpush.sendNotification(assinatura.subscription, JSON.stringify({
      title: 'DARPE — Você foi escalado!',
      body: `${grupo} — ${hospital} em ${new Date(data + 'T12:00:00').toLocaleDateString('pt-BR')}`
    }))
  } catch (e) { console.error('Erro ao notificar atendente:', e) }
}
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.confirmacao_aberta === true && dadosAntes?.confirmacao_aberta === false) {
    await supabaseAdmin.from('logs').insert([{
      usuario_nome: usuarioNome,
      acao: `Liberou escala: ${dadosAntes?.grupo} — ${dadosAntes?.data}`,
      tabela: 'escalas',
      registro_id: String(id),
      dados_antes: { confirmacao_aberta: false },
      dados_depois: { confirmacao_aberta: true },
    }])
  }
