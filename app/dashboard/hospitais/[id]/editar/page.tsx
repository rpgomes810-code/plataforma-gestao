...
export default function EditarHospital() {
  ...
  useEffect(() => {
    fetch(`/api/hospitais/${params.id}`)
      ...
      .then(data => {
        setForm({
          nome: data.nome || '', endereco: data.endereco || '',
          numero: data.numero || '', bairro: data.bairro || '',
          cidade: data.cidade || '', estado: data.estado || '',
          contato: data.contato || '', turno: data.turno || '',
          responsavel: data.responsavel || '', dia_semana: data.dia_semana || '',
          site: data.site || '', localizacao: data.localizacao || '',
          observacoes: data.observacoes || '', data_inicio_atividade: data.data_inicio_atividade || '',
        })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { alert('Erro ao atualizar hospital'); setLoading(false) }
  }
